"""M1. Executive Dashboard - 지주사 경영진 종합 대시보드"""
...
with col_f1:
    selected_bus = st.multiselect(
        "계열사 선택", options=list(BUSINESS_UNITS.keys()),
...
c1.metric("연결매출", f"{rev:,.0f} 억원", f"{rev_d:+,.0f}")
c2.metric("연결EBITDA", f"{ebitda_v:,.0f} 억원", f"{ebitda_d:+,.0f}")
c3.metric("연결영업이익", f"{ebit_v:,.0f} 억원", f"{ebit_d:+,.0f}")
c4.metric("연결CAPEX", f"{capex_v:,.0f} 억원", f"{capex_d:+,.0f}", delta_color="inverse")
c5.metric("계약잔고", f"{backlog_v:,.0f} 억원", f"{backlog_d:+,.0f}")
...
with col3:
    ...
    if not latest.empty:
        ...
        fig3 = px.bar(latest_summary, x="bu_name", y="margin", color="bu_name",
                      ...
                      text="margin",
                      color_discrete_map={v["name"]: v["color"] for v in BUSINESS_UNITS.values()})
...
# Gap Alert Table
st.subheader("Gap Alert - 계획 대비 미달 항목")
if not latest.empty:
    ...
    gap_df = gap_df[display_cols].rename(columns={
        "bu_name": "계열사", "revenue": "매출(실적)", "plan_revenue": "매출(계획)",
